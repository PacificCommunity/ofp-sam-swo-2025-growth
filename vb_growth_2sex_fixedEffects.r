...
# set working directory
    proj.dir = "C:/Users/nicholasd/HOME/SPC/SPC_SAM/2021_SC/SWO/Assessment/Data_Prep/model_ensemble/"
    setwd(proj.dir)

#________________________________________________________________________________________________________________________________________________________________________________________________________
# load data
    data.dt = fread("input_data/SWO age data for SPC _200421.csv")
...
# run stan
    ...
    burnin.prop = 0.2
    options(mc.cores = 3)
    file.name = c("stan_code/vb_growth_2sex_fixedEffects.stan") # path to stan file
...
# plot priors with posteriors
 png(filename = paste0(proj.dir,"plots/vb_growth_2sex_fixedEffects.png"), width = 16, height = 9, units = "in",  res = 300)
    par(mfrow=c(2,4))
...
# plot model fits
    ...
    png(filename = paste0(proj.dir,"plots/vb_growth_2sex_fixedEffects-mu.png"), width = 9, height = 9, units = "in",  res = 300)
    ...
    png(filename = paste0(proj.dir,"plots/vb_growth_2sex_fixedEffects-specific.png"), width = 9, height = 9, units = "in",  res = 300)
    plot(tmp.dt[,.(otolith_age,ljfl_cm)],type="n",ylim=c(0,300),xlab="Age (years)",ylab="LJFL (cm)",cex=1.5,cex.axis=1.5,cex.lab=1.5,las=1)
    ...
    # lines(a.vec,median(sub.chains$Linf_mu)*(1-exp(-median(sub.chains$k_mu)*(a.vec-median(sub.chains$t0)))),col="black",lwd=3)
    lines(1:20,109.319143026271+(241.178357853513-109.319143026271)*((1-exp(-0.190777127959981*(1:20-1)))/(1-exp(-0.190777127959981*(20-1)))),col="green",lwd=3)
    dev.off()
            
    save(par.chains,file=paste0(proj.dir,"stan_output/vb_growth_2sex_fixedEffects.RData"))
...
# make plots
# plot priors with posteriors
 png(filename = paste0(proj.dir,"plots/vb_growth_2sex_fixedEffects_t0prior.png"), width = 16, height = 9, units = "in",  res = 300)
    par(mfrow=c(2,4))
    for(target_variable in c("t0[1]","t0[2]","logk[1]","logk[2]","logLinf[1]","logLinf[2]","logSigma[1]","logSigma[2]"))
    {
        x.pad = 0.25*diff(range(fit.df[,target_variable]))
...
# plot model fits
    ...
    png(filename = paste0(proj.dir,"plots/vb_growth_2sex_fixedEffects_t0prior-mu.png"), width = 9, height = 9, units = "in",  res = 300)
    plot(tmp.dt[,.(otolith_age,ljfl_cm)],type="n",ylim=c(0,300),xlab="Age (years)",ylab="LJFL (cm)",cex=1.5,cex.axis=1.5,cex.lab=1.5,las=1)
    points(tmp.dt[,.(otolith_age,ljfl_cm)],pch=21,cex=1.25,bg="gray70")
    a.vec = seq(from=-5,to=25,length.out=100)
    for(i in 1:nrow(sub.chains))
    ...
    png(filename = paste0(proj.dir,"plots/vb_growth_2sex_fixedEffects_t0prior-specific.png"), width = 9, height = 9, units = "in",  res = 300)
    plot(tmp.dt[,.(otolith_age,ljfl_cm)],type="n",ylim=c(0,300),xlab="Age (years)",ylab="LJFL (cm)",cex=1.5,cex.axis=1.5,cex.lab=1.5,las=1)
    points(tmp.dt[,.(otolith_age,ljfl_cm)],pch=16,cex=1.25,col=c("blue","orange")[as.numeric(factor(tmp.dt$sex,levels=c("M","F")))])
    a.vec = seq(from=-5,to=25,length.out=100)
    for(i in 1:nrow(sub.chains))
    ...
    save(par.chains,file=paste0(proj.dir,"stan_output/vb_growth_2sex_fixedEffects_t0prior.RData"))
